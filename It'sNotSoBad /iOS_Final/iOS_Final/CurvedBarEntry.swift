...
struct MainBarEntry {
    ...
    var curvedSegments: [CurvedSegment] {
        ...
        controlPoint2 = CGPoint(x: origin.x + barWidth*2.5/10, y: origin.y)
        ...
        toPoint = CGPoint(x: origin.x + barWidth/2, y: origin.y)
        controlPoint1 = CGPoint(x: origin.x+barWidth/2, y: origin.y + barHeight)
        controlPoint2 = CGPoint(x: origin.x + barWidth*7.5/10, y: origin.y)
...
struct TopBubbleEntry {
    private let topBubbleRadius: CGFloat = 17.0
    private var curvedValue: CGFloat {
        //curve value to draw 1/4 of the circle using Bezier Function
